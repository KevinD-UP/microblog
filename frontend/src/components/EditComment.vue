...
export default {
    ...
    methods: {
        async getCommentaireById() {
            try {
                const response = await axios.get(
                    `http://localhost:5000/api/commentaire/${this.$route.params.id}`
                );
        ...
        submit: async function(id) {
            ...
            await axios
                .put(`http://localhost:5000/api/commentaires/edit/${id}`, {
                    idCommentaire: this.publications[0].idCommentaire,
                ...
                .then(
                    (response) => {
                        ...
                        this.$router.push({ path: `/commentary/${this.publications[0].idPublication}` });
                    },
                    (error) => {
                        console.log(error);
                    }
                );
            swal("Bien joué", "Commentaire édité", "success");
            this.$v.$reset();
            this.resetData();
        },
        async getDataUser() {
